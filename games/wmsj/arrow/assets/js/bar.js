var bar = {
	...
	destore: function() {
		...
		if (this.num >= this.beginBarTop && this.num <= (this.heights + this.beginBarTop)) {
			...
			setTimeout(function() {
				var overIndex = Math.floor(Math.random() * 3);
				window.over = [
						"我被射了一脸，还好中了" + bar.score + "个苹果！",
						"我膝盖中了一箭，还好中了" + bar.score + "个苹果！",
						"我被丘比特射中了，还好中了" + bar.score + "个苹果~~快来跟我搞基吧！"
					];

					window.confirm("拼死射中" + bar.score + "只苹果，快分享给小伙伴们吧！");
					document.title = window.over[overIndex];
